...
def make_cache_client(settings: Settings) -> CacheClient | None:
    """Create the exact-match cache client.

    Redis is an optional accelerator, not a hard dependency. If the cache is
    unavailable the platform must still start and operate (offline-first
    requirement), so connection failures degrade to a disabled cache (``None``)
    instead of raising and blocking startup.
    """
    try:
        ...
        logger.warning(f"Cache disabled - could not connect to Redis: {e}")
        return None
